...
int main(int argc, char *argv[]) { 
    ...
    printf("Se recomienda para una buena visualizacion y obtencion de resultados, workers desde los 7 en adelante\n");
    printf("Se asegura que con 15 workers los resultados son los esperados\n");
    printf("Desea continuar?\n");
    printf("S/N?\n");
    scanf("%c",&exe_option);
    while(1){

        if(exe_option == 'N' || exe_option == 'n'){
            return 1;
        } else if(exe_option == 'S' || exe_option == 's'){
            break;
        } else {
            printf("Ingrese S para salir o N para continuar\n");
            scanf("%c",&exe_option);
        }
    }
