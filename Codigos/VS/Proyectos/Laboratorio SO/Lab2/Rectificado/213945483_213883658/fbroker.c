...
//Salida: bool
//Descripción: verifica si el nombre del archivo contiene caracteres especiales
bool is_sepcial(const char *name){
    while (*name) {
        if (!isprint((unsigned char)*name) && !isspace((unsigned char)*name)) {
            return true;
        }
        name++;
    }
...
void write_csv(BMPImage *image, const char *csvname) {
    ...
    int type = image->type;
    char typestr[20]; // Asegúrate de que esto sea lo suficientemente grande
...
//Entrada: estructura BMPImage y cantidad de trabajadores
//Salida: arreglo de estructuras BMPImage
//Descripción: divide una imagen en n partes iguales dejando el residuo en la última parte
...
//Entrada: arreglo de estructuras BMPImage y cantidad de imagenes del arreglo
//Salida: BMPImage
//Descripción: combina las imagenes del arreglo en una sola imagen
...
//Entrada: arreglo de BMPImages, cantidad de workers, opcion de filtro, factor de saturacion y umbral de binarizacion
//Salida: arreglo de BMPImages
//Descripcion: funcion principal que se encarga de crear procesos segun cantidad de workers los cuales ejecutaran su respectivo programa y filtraran las 
//imagenes que les envie el proceso padre atraves de sus respectivos pipes
BMPImage** send_and_receive(BMPImage** imageSplit, int num_workers, int filter_opt, float saturation_fact, float threshold_bina) {
    ...
    for (int i = 0; i < num_workers; i++){ // crear hijos
        ...
        if(pid == -1){
            ...
            execv(argv[0],argv); // Ejecutar el programa worker
            exit(EXIT_FAILURE);
            ...
            imageSplit[i] = processed_image; // Guardar la imagen procesada en el arreglo
        }
