...
int agregar_arista(Grafo *G, int v, int w); //agrega una arista (v,w) al grafo
...
int agregar_arista(Grafo *G, int v, int w){
	if(is_lista_vacia(G->lisv[v])==1){
		insertar_nodo_ini(G->lisv[v],w);
		G->m=G->m+1;
		return 0;
	}
	if(is_lista_vacia(G->lisv[v])==0){
...
int buscar_arista(Grafo *G, int v, int w){
	if(buscar_nodo(G->lisv[v],w)==NULL)
		return 1;
	if(buscar_nodo(G->lisv[v],w)!=NULL)
		return 0;
}

int eliminar_arista(Grafo *G, int v, int w){
	if(buscar_arista(G,v,w)==1)
		return 1;
	if(buscar_arista(G,v,w)==0){
